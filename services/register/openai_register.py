...
def validate_otp(session: requests.Session, device_id: str, code: str):
    ...
    if resp is not None and resp.status_code == 200:
        return resp, ""
    sentinel_value, oai_sc = build_sentinel_token(session, device_id, "authorize_continue")
    headers["openai-sentinel-token"] = sentinel_value
    if oai_sc:
        session.cookies.set("oai-sc", oai_sc, domain=".sentinel.openai.com")
        session.cookies.set("oai-sc", oai_sc, domain="sentinel.openai.com")
    resp, error = request_with_local_retry(session, "post", f"{auth_base}/api/accounts/email-otp/validate", json={"code": code}, headers=headers, verify=False)
    return resp, error
...
class PlatformRegistrar:
    ...
    def _register_user(self, email: str, password: str, index: int) -> None:
        ...
        sentinel_value, oai_sc = build_sentinel_token(self.session, self.device_id, "username_password_create")
        headers["openai-sentinel-token"] = sentinel_value
        if oai_sc:
